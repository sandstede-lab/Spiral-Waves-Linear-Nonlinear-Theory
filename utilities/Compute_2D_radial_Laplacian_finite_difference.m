%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute 2D-radial finite-difference Laplacian - Neumann bcs on r=[r1,r2]
% Inputs: [r1,r2] - annulus, Nrh - number of mesh points
% Outputs: r - mesh, L - Laplacian, Dx - 1D differentiation matrix,
% Note: returned matrices are sparse
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Rn,L,Dxn] = Compute_2D_radial_Laplacian_finite_difference(Nrh,r1,r2,order)

hxn = (r2-r1)/(Nrh-1);
rn = linspace(r1,r2,Nrh)';
if r1<1e-08
	rn(1) = 1;
end
...
case '4'
    ...
    Dxn = (Dxn - Dxn')/12; Dxn(1:2,:) = 0; Dxn(end-1:end,:) = 0; Dxn(2,1:4) = [-2, -3, 6, -1]/6; Dxn(end-1,end-3:end) = [1, -6, 3, 2]/6; 
    ...
    D2xn(1:2,:) = 0; D2xn(end-1:end,:) = 0; D2xn(2,1:3) = [1, -2, 1];  % Neumann boundary conditions: use 2nd order at boundary
    D2xn(end-1,end-2:end) = [1, -2, 1]; D2xn(1,1:2) = [-2,2]; D2xn(end,end-1:end) = [2,-2];
