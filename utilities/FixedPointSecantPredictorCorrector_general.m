function [F, J]  = FixedPointSecantPredictorCorrector_general(sol,sol1,sol0,par,numPar,contPar,phase_cond,fh)
...
F = zeros(size(sol));
[Fu,Ju] = fh(u,par,numPar,phase_cond); % evaluate system at predictor u and p
F(1:end-1) = Fu;
...
if nargout > 1 % Redefine the Jacobian - need to find the Jacobian wrt the continuation parameter
    ...
    Fu2 = fh(u,parpe,numPar,phase_cond);    
